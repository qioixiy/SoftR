...
D3DApp::~D3DApp()
{
	...
	ReleaseCOM(m_alphaDisableBlendingState);
}
...
void D3DApp::OnResize()
{
	...
	//该格式中，每个像素为4字节（32位），其中深度值占24位，并映射到[0,1]之间。模板值为8位，为位于[0,255]中的整型
	...
	g_env->d3d_DepthStencilView = mDepthStencilView;
	g_env->d3d_RenderTargetView = mRenderTargetView;
...
bool D3DApp::InitDirect3D()
{
	...
	g_env->d3d_device = md3dDevice;
	g_env->d3d_context = md3dImmediateContext;
	...
	g_env->d3d_context->OMSetBlendState(m_alphaEnableBlendingState, factor, 0xffffffff);
	
	/*
	D3D11_BLEND_DESC blendStateDescription;
	blendStateDescription.RenderTarget[]
	g_env->d3d_context->OMGetBlendState()
	*/
	return true;
}

void D3DApp::create_states()
{
	...
	g_env->d3d_device->CreateDepthStencilState(&depthDisabledStencilDesc, &m_depthDisabledStencilState);
	...
	g_env->d3d_device->CreateDepthStencilState(&depthStencilDesc, &m_depthStencilState);
	...
	g_env->d3d_device->CreateBlendState(&blendStateDescription, &m_alphaDisableBlendingState);
}
